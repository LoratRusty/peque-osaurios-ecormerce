...
<body class="bg-gradient-to-br from-pink-50 via-purple-50 to-rose-50 min-h-screen flex font-sans text-gray-800">
    ...
    <button 
        ...
        <!-- Usuario Info -->
        <a href="<?php echo e(route('profile.edit')); ?>" class="block px-6 py-4 bg-white/50 backdrop-blur-sm mx-4 mt-4 rounded-xl border border-white/20 shadow-sm cursor-pointer hover:bg-white/70 hover:shadow-lg hover:scale-[1.02] transition transform duration-200">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <p class="font-semibold text-gray-800"><?php echo e(session('usuario.name')); ?></p>
                    <p class="inline-block w-auto text-xs text-gray-600 capitalize bg-gradient-to-r from-pink-100 to-purple-100 px-2 py-1 rounded-full">
                        Rol: <strong><?php echo e(getRolNombre(auth()->user()->tipo)); ?></strong>
                    </p>
                </div>
            </div>
        </a>
        ...
        <nav class="flex-grow mt-6 px-4 space-y-2 relative z-10 overflow-y-auto">
            ...
            <?php $__currentLoopData = $menuItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php if(in_array(auth()->user()->tipo, $item['roles'])): ?>
                    <a href="<?php echo e(route($item['route'])); ?>" 
                    class="group flex items-center px-4 py-3 rounded-xl transition-all duration-300 nav-item-hover
                    <?php echo e(request()->routeIs($item['route'] . '*') ? 'nav-item-active' : 'text-gray-700'); ?>">
                        <div class="flex items-center space-x-3">
                            <div class="w-8 h-8 flex items-center justify-center rounded-lg
                                <?php echo e(request()->routeIs($item['route'] . '*') ? 'bg-white/20' : 'bg-pink-100 group-hover:bg-pink-200'); ?> 
                                transition-colors duration-300">
                                <svg class="w-5 h-5 <?php echo e(request()->routeIs($item['route'] . '*') ? 'text-white' : 'text-pink-600'); ?>" 
                                    fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="<?php echo e($item['icon']); ?>"></path>
                                </svg>
                            </div>
                            <span class="font-semibold text-sm"><?php echo e($item['text']); ?></span>
                        </div>
        ...
        <div class="px-6 py-4 mt-auto bg-white/50 backdrop-blur-sm mx-4 mb-4 rounded-xl border border-white/20 shadow-sm">
            <p class="text-xs text-gray-600">Admin Panel -Pequeñosaurios &copy; <?php echo e(date('Y')); ?>.</p>
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">
                            <?php echo $__env->yieldContent('title', 'Panel de Administración'); ?>
                        </h1>
...
<?php /**PATH C:\xampp-actual\htdocs\pequenosaurios\resources\views/layouts/admin.blade.php ENDPATH**/ ?>